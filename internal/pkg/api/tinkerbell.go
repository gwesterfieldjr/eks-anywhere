...
func WithTinkerbellHegelURL(value string) TinkerbellFiller {
	return func(config TinkerbellConfig) error {
		for _, m := range config.machineConfigs {
			templateOverride, err := anywherev1.NewTinkerbellTemplateFromString(m.Spec.TemplateOverride)
			if err != nil {
				return fmt.Errorf("failed to set tinkerbell hegel url: %v", err)
			}

			for _, task := range templateOverride.Tasks {
				if task.Name == m.Name {
					for _, action := range task.Actions {
						if action.Name == "add-tink-cloud-init-config" {
							contents := action.Environment["CONTENTS"]
							action.Environment["CONTENTS"] = strings.ReplaceAll(contents, anywherev1.TinkerbellDefaultMachineTemplateHegelURL, value)
						}
					}
				}
			}

			m.Spec.TemplateOverride, err = templateOverride.ToString()
			if err != nil {
				return fmt.Errorf("failed to set tinkerbell hegel url: %v", err)
			}
		}
...
func WithTemplateForAllTinkerbellMachines(value string) TinkerbellFiller {
	return func(config TinkerbellConfig) error {
		for _, m := range config.machineConfigs {
			templateOverride, err := anywherev1.NewTinkerbellTemplateFromString(m.Spec.TemplateOverride)
			if err != nil {
				return fmt.Errorf("failed to set tinkerbell machine template for all machines: %v", err)
			}
